#!/bin/bash
# cloud-init: community-72k Labels-ONLY re-run (AccessDenied retry).
exec > >(tee /home/ubuntu/community_72k_relabels_userdata.log) 2>&1
set -euo pipefail
...
echo "=== community_72k RELABELS userdata start $(date -u) ==="
...
chmod +x "$PIPE/aws/community_72k_relabels_chain.sh" || true
...
sudo -u ubuntu -H env \
  ...
  bash -lc "cd $PIPE && bash aws/community_72k_relabels_chain.sh"
