#!/bin/bash
# cloud-init: pull OCR pilot v2 code + sample from S3, run chain, upload, shutdown.
exec > >(tee /home/ubuntu/ocr_pilot_v2_userdata.log) 2>&1
set -euo pipefail
export BUCKET=aof-owlv2-102516364259
export PREFIX=ocr_pilot_v2
...
PIPE=$ROOT/pipeline
mkdir -p "$PIPE/data/posters" "$PIPE/aws" "$PIPE/data/qa/${PREFIX}"
...
aws s3 cp "s3://${BUCKET}/${PREFIX}/code/ocr_pilot_v2_chain.sh" "$PIPE/aws/ocr_pilot_v2_chain.sh"
chmod +x "$PIPE/aws/ocr_pilot_v2_chain.sh"
...
sudo -u ubuntu -H bash -lc "cd $PIPE && bash aws/ocr_pilot_v2_chain.sh"
