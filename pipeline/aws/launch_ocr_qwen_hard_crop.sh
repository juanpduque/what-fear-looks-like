#!/bin/bash
# Launch EC2 GPU for hard-set text-det → crop → Qwen 2B/7B OCR.
# Prerequisites: bash aws/stage_ocr_qwen_hard_crop.sh
#
#   MODELS=qwen,qwen7 bash aws/launch_ocr_qwen_hard_crop.sh
set -euo pipefail
...
NAME_TAG="${NAME_TAG:-aof-ocr-qwen-hard-crop}"
MAX_N="${MAX_N:-120}"
...
USERDATA="$PIPE/aws/ocr_qwen_hard_crop_userdata.sh"
if [ ! -f "$USERDATA" ]; then
...
echo "Pull:     bash aws/pull_ocr_qwen_hard_crop.sh"
