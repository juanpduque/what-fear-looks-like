#!/bin/bash
# Hard-set Kimi-VL OCR (EC2 GPU g5.2xlarge): Instruct on staged posters.
# Periodic S3 sync, DONE, halt.
set -euo pipefail
export BUCKET="${BUCKET:-aof-owlv2-102516364259}"
export PREFIX="${PREFIX:-ocr_kimi_hard}"
...
# Prefer 4bit on 24GB A10G (16B MoE weights); override with KIMI_LOAD=bf16
export KIMI_LOAD="${KIMI_LOAD:-4bit}"
export KIMI_HF_ID="${KIMI_HF_ID:-moonshotai/Kimi-VL-A3B-Instruct}"
...
# Model card: transformers~=4.48.2 / 4.51.x; keep recent enough for MoE VL
$PYTHON -m pip -q install -U 'transformers>=4.48.2,<=4.51.3' accelerate pillow pandas numpy \
...
echo "MODELS=$MODELS N=$N KIMI_LOAD=$KIMI_LOAD KIMI_HF_ID=$KIMI_HF_ID"

result_tag_for() {
  case "$1" in
    kimi) echo "kimi-vl-hard" ;;
    *) echo "${1}-hard" ;;
  esac
}
...
for m in "${MODEL_ARR[@]}"; do
  ...
  if [ "$rc" -ne 0 ]; then
    echo "ERROR: model $m failed with exit=$rc — not switching to API; writing FAIL marker"
