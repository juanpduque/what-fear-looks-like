#!/bin/bash
# cloud-init: pull hard Kimi-VL OCR code + sample from S3, run chain, upload, shutdown.
exec > >(tee /home/ubuntu/ocr_kimi_hard_userdata.log) 2>&1
set -euo pipefail
export BUCKET=aof-owlv2-102516364259
export PREFIX=ocr_kimi_hard
...
aws s3 cp "s3://${BUCKET}/${PREFIX}/code/ocr_kimi_hard_chain.sh" "$PIPE/aws/ocr_kimi_hard_chain.sh"
chmod +x "$PIPE/aws/ocr_kimi_hard_chain.sh"
...
sudo -u ubuntu -H bash -lc "cd $PIPE && bash aws/ocr_kimi_hard_chain.sh"
