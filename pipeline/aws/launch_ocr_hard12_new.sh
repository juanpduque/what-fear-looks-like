#!/bin/bash
# Launch Spot g5.2xlarge for hard12 GLM / Unlimited / DeepSeek-OCR-2.
# Prerequisites: bash aws/stage_ocr_hard12_new.sh
#
#   MODELS=glm,unlimited,deepseek2 bash aws/launch_ocr_hard12_new.sh
#   MARKET=on-demand bash aws/launch_ocr_hard12_new.sh
set -euo pipefail
...
NAME_TAG="${NAME_TAG:-aof-ocr-hard12-new}"
MAX_N="${MAX_N:-120}"
...
USERDATA="$PIPE/aws/ocr_hard12_new_userdata.sh"
if [ ! -f "$USERDATA" ]; then
...
if [ "$N_LOCAL" -lt 1 ]; then
  echo "ERROR: empty sample_ids.txt — run stage_ocr_hard12_new.sh first"; exit 1
fi
...
if [ -n "$SUBNET_ID" ]; then
  set +e
  IID=$(launch_once "$SUBNET_ID" 2>/tmp/ocr_hard12_new_launch.err)
  rc=$?
  set -e
  if [ "$rc" -ne 0 ] || [ -z "$IID" ] || [ "$IID" = "None" ]; then
    echo "ERROR: launch failed on forced SUBNET_ID=$SUBNET_ID"
    cat /tmp/ocr_hard12_new_launch.err 2>/dev/null | tail -40 || true
    exit 1
  fi
  ...
  for az in "${AZ_ARR[@]}"; do
    ...
    set +e
    IID=$(launch_once "$sn" 2>/tmp/ocr_hard12_new_launch.err)
    rc=$?
    set -e
    ...
    tail -5 /tmp/ocr_hard12_new_launch.err 2>/dev/null || true
    IID=""
  done
...
echo "Pull:     bash aws/pull_ocr_hard12_new.sh"
