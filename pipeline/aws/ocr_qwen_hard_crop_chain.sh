#!/bin/bash
# Hard-set: open-source text det → crop → Qwen OCR (2B then 7B 4bit) on crops.
# Periodic S3 sync, DONE, halt.
set -euo pipefail
export BUCKET="${BUCKET:-aof-owlv2-102516364259}"
export PREFIX="${PREFIX:-ocr_qwen_hard_crop}"
...
# Classic PP-OCR detection (prefer); surya as fallback later if needed
echo "--- install paddleocr (det) ---"
set +e
$PYTHON -m pip -q install -U paddlepaddle paddleocr opencv-python-headless 2>&1 | tail -20
PADDLE_RC=${PIPESTATUS[0]}
set -e
echo "paddleocr pip exit=$PADDLE_RC"
...
sync_results() {
  if [ -d "data/qa/${PREFIX}" ]; then
    aws s3 sync "data/qa/${PREFIX}/" "s3://${BUCKET}/${PREFIX}/results/" \
      --exclude "*" \
      --include "*.csv" --include "*.txt" --include "*.log" --include "*.json" \
      --include "DONE" --include "crops/*" || true
  fi
}
...
result_tag_for() {
  case "$1" in
    qwen) echo "qwen2b-crop" ;;
    qwen7) echo "qwen7b-crop" ;;
    *) echo "${1}-crop" ;;
  esac
}

clear_hf_cache() {
  echo "--- clearing HF cache under $HF_HOME ---"
...
# ── detect → crop ───────────────────────────────────────────────────────────
echo "--- text det → crop $(date -u) ---"
# Disable OneDNN for paddle text det on DLAMI
export FLAGS_use_mkldnn=0
export FLAGS_onednn=0
set +e
$PYTHON -u crop_posters_text_det.py \
...
DET_RC=$?
set -e
echo "crop_posters_text_det (paddle) exit=$DET_RC"
NEED_SURYA=0
if [ "$DET_RC" -ne 0 ]; then
  NEED_SURYA=1
elif [ -f "data/qa/${PREFIX}/det_meta.json" ]; then
  REAL_N=$($PYTHON -c "import json; print(json.load(open('data/qa/${PREFIX}/det_meta.json')).get('n_real_crops',0))")
  if [ "${REAL_N:-0}" -eq 0 ]; then
    NEED_SURYA=1
  fi
fi
if [ "$NEED_SURYA" = "1" ]; then
  echo "WARNING: paddle produced no usable crops; trying surya"
  set +e
  $PYTHON -m pip -q install -U surya-ocr 2>&1 | tail -15
  $PYTHON -u crop_posters_text_det.py \
    --ids-file "data/qa/${PREFIX}/sample_ids.txt" \
    --posters-dir data/posters_hard \
    --out-dir "data/qa/${PREFIX}" \
  ...
  DET_RC=$?
  set -e
  echo "surya crop exit=$DET_RC"
fi

# Ensure every id has a crop (full-image fallback)
mkdir -p "data/qa/${PREFIX}/crops"
while read -r pid; do
  [ -z "$pid" ] && continue
  if [ ! -f "data/qa/${PREFIX}/crops/${pid}.jpg" ]; then
    echo "missing crop for $pid — copying full poster"
    cp -f "data/posters_hard/${pid}.jpg" "data/qa/${PREFIX}/crops/${pid}.jpg"
  fi
done < "data/qa/${PREFIX}/sample_ids.txt"

N_CROPS=$(ls "data/qa/${PREFIX}/crops"/*.jpg 2>/dev/null | wc -l | tr -d ' ')
echo "crops ready: $N_CROPS"
if [ -f "data/qa/${PREFIX}/det_meta.json" ]; then
  cat "data/qa/${PREFIX}/det_meta.json"
fi
sync_results
echo "[checkpoint] synced after det/crop"
...
for m in "${MODEL_ARR[@]}"; do
  ...
  echo "--- disk before $m ---"
  df -h / /home/ubuntu 2>/dev/null || df -h /
  echo "--- run model=$m result=$RESULT_MODEL on crops $(date -u) ---"
  set +e
  $PYTHON -u pilot_ocr_models.py \
    --n "$N" \
    --ids-file "data/qa/${PREFIX}/sample_ids.txt" \
    --out-dir "data/qa/${PREFIX}" \
    --posters-dir "data/qa/${PREFIX}/crops" \
    --models "$m" \
