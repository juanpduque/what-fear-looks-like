#!/bin/bash
# Launch EC2 GPU for hard-set Qwen 2B vs 7B OCR mini-pilot.
# Prerequisites: bash aws/stage_ocr_qwen_hard.sh
#
#   MODELS=qwen,qwen7 bash aws/launch_ocr_qwen_hard.sh
set -euo pipefail
...
NAME_TAG="${NAME_TAG:-aof-ocr-qwen-hard}"
MAX_N="${MAX_N:-120}"
...
USERDATA="$PIPE/aws/ocr_qwen_hard_userdata.sh"
if [ ! -f "$USERDATA" ]; then
...
ARGS=(
  ...
  # 7B HF download needs more than default DLAMI root (~50–80GB fills with 2B+7B)
...
echo "Pull:     bash aws/pull_ocr_qwen_hard.sh"
