#!/bin/bash
# cloud-init: pull Qwen-homolog OCR code + sample from S3, run chain, upload, shutdown.
exec > >(tee /home/ubuntu/ocr_qwen_homolog_userdata.log) 2>&1
set -euo pipefail
export BUCKET=aof-owlv2-102516364259
export PREFIX=ocr_qwen_homolog
...
PIPE=$ROOT/pipeline
mkdir -p "$PIPE/data/posters_homolog" "$PIPE/aws" "$PIPE/data/qa/${PREFIX}"
...
aws s3 cp "s3://${BUCKET}/${PREFIX}/code/ocr_qwen_homolog_chain.sh" "$PIPE/aws/ocr_qwen_homolog_chain.sh"
chmod +x "$PIPE/aws/ocr_qwen_homolog_chain.sh"
...
sudo -u ubuntu -H bash -lc "cd $PIPE && bash aws/ocr_qwen_homolog_chain.sh"
