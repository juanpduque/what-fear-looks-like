#!/bin/bash
# Stage hard-set posters + code for Kimi-VL OCR on the 12 hard ids.
# Reuses ocr_qwen_hard sample ids / poster_sources (homolog > original_up > original > w342).
#
# Usage:
#   bash aws/stage_ocr_kimi_hard.sh
set -euo pipefail
...
echo "sample_n=$N_LOCAL (kimi hard from ${SRC_PREFIX})"
...
cp -f aws/ocr_kimi_hard_chain.sh "$STAGE/code/ocr_kimi_hard_chain.sh"
cp -f aws/ocr_kimi_hard_userdata.sh "$STAGE/code/ocr_kimi_hard_userdata.sh"
...
aws s3 cp "$STAGE/code/ocr_kimi_hard_chain.sh" "s3://${BUCKET}/${PREFIX}/code/ocr_kimi_hard_chain.sh"
aws s3 cp "$STAGE/code/ocr_kimi_hard_userdata.sh" "s3://${BUCKET}/${PREFIX}/code/ocr_kimi_hard_userdata.sh"
...
echo "Siguiente: MODELS=kimi bash aws/launch_ocr_kimi_hard.sh"
