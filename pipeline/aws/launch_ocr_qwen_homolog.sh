#!/bin/bash
# Launch EC2 GPU for Qwen OCR on homolog posters (same sample as ocr_pilot_v2).
# Prerequisites: bash aws/stage_ocr_qwen_homolog.sh
#
#   MODELS=qwen bash aws/launch_ocr_qwen_homolog.sh
set -euo pipefail
...
MODELS="${MODELS:-qwen}"
NAME_TAG="${NAME_TAG:-aof-ocr-qwen-homolog}"
MAX_N="${MAX_N:-120}"
...
USERDATA="$PIPE/aws/ocr_qwen_homolog_userdata.sh"
if [ ! -f "$USERDATA" ]; then
...
echo "Pull:     bash aws/pull_ocr_qwen_homolog.sh"
