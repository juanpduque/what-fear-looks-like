#!/bin/bash
# cloud-init: vision gap multi-pipeline (faces/attrs/clip/census/typo/medium/seg).
exec > >(tee /home/ubuntu/vision_gap_userdata.log) 2>&1
set -euo pipefail
...
echo "=== vision_gap userdata start $(date -u) ==="
ROOT=/home/ubuntu/aof
PIPE=$ROOT/pipeline
mkdir -p "$PIPE/data/qa/vision_gap" "$PIPE/aws" "$PIPE/models" "$PIPE/data/posters"
...
if aws s3 cp "s3://${BUCKET}/${PREFIX}/ENV" /tmp/vision_gap_env 2>/dev/null; then
  set -a
  # shellcheck disable=SC1091
  source /tmp/vision_gap_env
  set +a
fi
...
chmod +x "$PIPE/aws/vision_gap_chain.sh" || true
...
sudo -u ubuntu -H env \
  ...
  bash -lc "cd $PIPE && bash aws/vision_gap_chain.sh"
