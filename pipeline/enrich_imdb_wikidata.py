...
def main() -> None:
    ...
    ap.add_argument(
        "--ids",
        default="",
        help="CSV with an id column (e.g. gap list); default = posters.csv corpus",
    )
    ...
    if args.ids:
        ...
        pool = load_ids_file(ids_path)
        print(f"ids from {ids_path}: {len(pool):,}")
    else:
        if not POSTERS.exists():
            raise SystemExit(f"missing {POSTERS}")
        pool = load_corpus()
        print(f"corpus ids: {len(pool):,}")

    todo = [i for i in pool if not sidecar.get(i, "").startswith("tt")]
    if args.limit:
        todo = todo[: args.limit]
    print(f"without imdb_id: {len(todo):,}")
